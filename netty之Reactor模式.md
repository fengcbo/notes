...
Reactor模式将应用程序的功能分解到独立的类中。比如，Logging Server中有两个独立的类：一个用于建立连接、另一个用于接受和处理收到的日志记录。这种分离使得建立连接的类对于各种面向连接的服务(比如：文件传输、远程登录以及视频点播)都是可重用的。因此，修改以及扩展日志服务的功能只会影响事件处理器类的实现。

3. 提高应用程序的可移植性

Initiation Dispatcher的接口是可复用的，它独立于执行事件分发的操作系统调用。这些系统调用检测并报告一个或多个事件的发生，这些事件可能同时发生在多个事件源上。事件的常见来源可能包括I/O句柄、计时器和同步对象。在UNIX平台上，事件多路复用系统调用称为select和poll。在Win32 API中，WaitForMultipleObjects系统调用执行事件分解。

4. 提供粗粒度的并发控制

Reactor模式将事件处理程序的调用序列化为在一个进程或线程内的事件多路复用和分派级别。Initiation Dispatcher 级别的序列化通常能够在应用程序处理中消除更复杂的同步以及锁的需求。

### Reactor模式的缺点

1. 适用性受限

Reactor模式只有在操作系统支持句柄或文件描述符的时才能有效应用。Reactor模式可以在Initiation Dispatcher使用多线程。每个handle一个线程。无论何时handle上有事件到达，与之关联的线程会读取这个时间，并将它放入到一个队列，initiation dispatcher会并行的处理这个队列。但是，这种设计不是很高效，因为所有的Event Handle都串行执行，因此，会增加同步操作以及线程上下文切换成本而没有增加并发。

2. 没有优先级

在一个单线程的应用中，当在执行的时候，Event Handler是没有优先级的。这意味着Event Handler不能针对特定的Handle执行阻塞操作，因为这会阻塞住整个程序，并且会阻止客户端连接到其他Handle。

3. 调试困难

使用Reactor模式的应用程序很难调试，这是因为基础框架与应用程序特定的处理器的回调方法之间控制的反向流是震荡的。